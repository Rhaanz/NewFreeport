...
int32 Mob::GetActSpellDamage(uint16 spell_id, int32 value, Mob* target) {
	...
	int chance = 0;
	float SpellRatio = RuleR(Spells, SpellRatio);
	int32 SpellBonus = (itembonuses.SpellDmg * SpellRatio);
...
int32 Mob::GetActDoTDamage(uint16 spell_id, int32 value, Mob* target) {
	...
	int32 extra_dmg = 0;
	int16 chance = 0;
	float DotRatio = RuleR(Spells, DotRatio);
	float DotBonus = (itembonuses.SpellDmg * DotRatio);
	int total_cast_time = 0;
	...
	if (!spells[spell_id].good_effect && chance > 0 && (zone->random.Roll(chance))) {
		...
		extra_dmg = target->GetFcDamageAmtIncoming(this, spell_id) +
			...
			//extra_dmg += (itembonuses.SpellDmg * dotratio);
		if (total_cast_time > 1000) {
			value += (value * DotBonus) / 100.0f;
		}
...
int32 Mob::GetActSpellHealing(uint16 spell_id, int32 value, Mob* target) {
	...
	if (spells[spell_id].buff_duration < 1) {
		...
		//value += (itembonuses.HealAmt*healratio); //Item Heal Amt Add before critical
